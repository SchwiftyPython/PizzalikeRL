    ...
    private void PlacePlayer() {
        if (GameManager.Instance.PlayerInStartingArea) {
            ...
            if (_currentArea != null) {
                _playerSprite.transform.position = new Vector3(_currentArea.Height / 2, _currentArea.Width / 2);
                _player.CurrentPosition = new Vector3(_currentArea.Height / 2, _currentArea.Width / 2);
                _currentArea.AreaTiles[_currentArea.Height / 2, _currentArea.Width / 2].SetPresentEntity(_player);
            }
        }
        else {
